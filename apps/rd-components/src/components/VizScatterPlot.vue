...
<script>
import { select, selectAll, scaleBand, axisBottom, max, min, scaleLinear, axisLeft, schemePuBuGn, scaleOrdinal } from 'd3'
const d3 = { select, selectAll, scaleBand, axisBottom, max, min, scaleLinear, axisLeft, schemePuBuGn, scaleOrdinal }
...
export default {
  ...
  computed: {
    ...
    xAxisMin () { 
      return this.xMin ? this.xMin : d3.min(this.chartData, row => row[this.xvar])
    },
    xAxisMax () {
      return this.xMax ? this.xMax : d3.max(this.chartData, row => row[this.xvar])
    },
    yAxisMin () {
      return this.yMin ? this.yMin : d3.min(this.chartData, row => row[this.yvar])
    },
    yAxisMax () {
    ...
    palette () {
      if (this.group !== '') {
        ...
        const range = Object.keys(this.pointFillPalette).length
          ? Object.keys(this.pointFillPalette).map(key => this.pointFillPalette[key])
    ...
    legendData () {
      if (this.group !== '') {
        const groups = [...new Set(this.chartData.map(row => row[this.group]))];
