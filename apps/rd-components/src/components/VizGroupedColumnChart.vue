...
<script>
import { select, selectAll, scaleBand, axisBottom, max, min, scaleLinear, axisLeft, group, scaleOrdinal, schemePuBuGn } from 'd3'
const d3 = { select, selectAll, scaleBand, axisBottom, max, min, scaleLinear, axisLeft, group, scaleOrdinal, schemePuBuGn }
  ...
  computed: {
    ...
    palette () {
      ...
      const range = Object.keys(this.columnFillPalette).length
        ? Object.keys(this.columnFillPalette).map(key => this.columnFillPalette[key])
    ...
    legendData () {
      const data = new Array(...this.subCategories).map(value => [value, this.palette(value)]);
